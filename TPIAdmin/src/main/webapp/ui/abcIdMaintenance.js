...
var begin = (($scope.currentPage - 1) * $scope.numPerPage), end = begin + $scope.numPerPage;
	...
	$scope.saveTrigger = function(){
				...
				$http.get('/TPIAdmin/app/environmentInfoForParam?paramName=TRIGGER_API')
				.success(function (response, status, headers, config) {
					 //$scope.grouplist = angular.copy(response);
				//	$scope.idd=angular.copy(response.id); 
				...
				.then(function(response) {
					// $scope.myWelcome = response;
					$scope.urlValue = response.data.paramVal;
	...
	$scope.higherEnv = function(){
		
		
		var params = {
				  rowLine : [
					  $scope.value,$scope.testIsaID,$scope.testIsaQualifier,$scope.testGsId,$scope.productionIsaID,$scope.productionIsaQualifier,$scope.productionGsId
				  ]
		...
		var urll = $scope.urlValue + 'ABCIDMAINTENANCE';
				$http.post(urll,urlParams,{
