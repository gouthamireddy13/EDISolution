...
$scope.resetDilimeters();

$scope.segmentDelimiter = item.segmentDelimiter.id;
$scope.elementDelimiter = item.elementDelimiter.id;
	$scope.compositeElementDelimiter = item.compositeElementDelimiter.id;
	if(item.repeatDelimiter == undefined || item.repeatDelimiter == null){	
		$scope.repeatDelimiter = null;
		$scope.disableDilimeters();
	}
	else{
		$scope.repeatDelimiter = item.repeatDelimiter.id;
	}
...
if(item.lightWellPartner!=null)
	...
	//$scope.lwID = item.lightWellPartner.id;
	$scope.lightWellPartner.id = item.lightWellPartner.id;
	  ...
	  $scope.products = ['coffee', 'beer', 'wine', 'tea', 'milk'];
	        ...
	        modalInstance.result.then(function (selectedItems) {
	        	...
	        	// console.log('eventX found on Controller1 $scope');
	             ...
	             .error(function (data, status, header, config) {
					...
					{
					 var dir1={
							 id:$scope.transdObj.direction.id
							 
					 };
					 var doc1=
						 {
							 id: $scope.transdObj.document.id
						 };
					 var versn1=
						 {
							id: $scope.transdObj.version.id
						 };
					 var objn=
						 {direction:dir1,
							 document:doc1,
					 ...
					 angular.forEach( $scope.transactionList, function (item5) {
						 if(item5.direction.id==dir1.id&&item5.document.id==doc1.id&&item5.version.id==versn1.id)
							 {
							 var index1 = $scope.transactionList.indexOf(item5);
						    	$scope.transactionList.splice(index1, 1) 
		...
		$scope.protocolList = function(dblist)
		{
			...
			$scope.directionList = function(dbList)
			{
				...
				$scope.value = "";
				$scope.showName1=function(list,obj)
				{
					...
					if(obj!=undefined){
						...
						if(tempsc.length!=0)
							{
							$scope.value = tempsc[0].documentDescription;
							}
					}
				...
				$scope.shouldShow = function(list){
						...
						return (list.directionCode != 'ALL' && list.directionCode != 'U' );
		...
		$scope.addTransaction=function(flag,transaction)
		{
			...
			var temp = $filter("filter")($scope.transactionList, objn,true);
			if(temp.length!=0)
				{
		...
		$scope.transEdit=false;
			...
			$scope.protoval=true;
			 ...
			 $scope.validateName="";
				$scope.validName=false;
				$scope.emptyNm = false;
				$scope.checktppNameandSubmit = function(name,contactflag,transactionflag,delimiterflag,flag)
				{
					...
					if($scope.onlyNew)
					 ...
					 .then(function(response) {
					        ...
					        if($scope.validateName!="true")
					        	...
					        	$scope.checkids2(contactflag,transactionflag,delimiterflag,flag);
					        	}
					        	else{
					        		$scope.savetpp(contactflag,transactionflag,delimiterflag,flag);
					        	}
					        	}
					        else
					        	{
					...
					$scope.checkids2 = function(contactflag,transactionflag,delimiterflag,flag)
					{
			...
			$scope.savetpp = function(contactflag,transactionflag,delimiterflag,flag){
				...
				var tempproto=[];
				 ...
				 var typeobj=
					...
					$scope.disablesave=true;
					 ...
					 if($scope.repeatDelimiter != undefined || $scope.repeatDelimiter!= null){
					 var submitobj= 
					 {
					     id:$scope.id,
						 type:typeobj,
						 ...
						 segmentDelimiter:{id:$scope.segmentDelimiter},
							 elementDelimiter:{id:$scope.elementDelimiter},
								 compositeElementDelimiter:{id:$scope.compositeElementDelimiter},
									
								 repeatDelimiter:{id:$scope.repeatDelimiter} ,
