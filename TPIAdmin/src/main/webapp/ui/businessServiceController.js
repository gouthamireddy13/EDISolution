...
/*	$scope.constructValue = function()
	...
	*/
	...
	var doempty=false;
		...
		{
		searchobj.businessSubUnit=search2;
		doempty=true;
		}
...
var begin = (($scope.currentPage - 1) * $scope.numPerPage), end = begin + $scope.numPerPage;
	...
	/*  $scope.showNm=function(input)
	  ...
	  $scope.showName=function(list,obj)
		...
		$scope.shouldShow = function(list,obj){
			...
			if(obj!=undefined){
				...
				if(tempsc.length!=0)
					{
					if(tempsc[0].name.indexOf("Intercompany")!=-1){
                      $scope.shouldReturn = function(list){
							
							
							
							return (list.name != 'Intercompany' );
	...
	$scope.saveABC = function(){
		...
		var submitObj=
			{   
				...
				serviceLevelId : $scope.serviceUserId,
		 ...
		 .then(function(response) {
			 angular.forEach(tableList, function (item) {
				 ...
				 {      
						 ...
						 serviceCategory:item.serviceCategory.serviceCategory.name,
						 businessUnit : item.serviceCategory.businessUnit.name,
