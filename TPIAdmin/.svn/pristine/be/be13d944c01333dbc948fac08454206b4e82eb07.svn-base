...
var begin = (($scope.currentPage - 1) * $scope.numPerPage), end = begin + $scope.numPerPage;
	  ...
	  $scope.showName=function(list,obj)
		{
			...
			$scope.sourceID = null;
			$scope.destinationID = null;
			if($scope.value == 'Customer/Standard IN' || $scope.value == 'Customer/Standard OUT' || $scope.value == 'Supplier/Standard IN' || $scope.value == 'Supplier/Standard OUT'){
	...
	$scope.checkIDsandSubmit = function(flag)
	{
		...
		$scope.validId=false;
		 ...
		 $http.get('/TPIAdmin/app/sd/sdServiceAccess/exists?sourceId='+$scope.sourceID+'&destId='+$scope.destinationID+'&serviceCategoryDefId='+$scope.serviceCategory+'&servicePreamble='+$scope.servicePreamble+'&serviceTypeId='+$scope.serviceType)
	     .success(function (response, status, headers, config) {
	...
	$scope.saveABC = function(flag){
		$scope.ErrorMsg=true;
		if($scope.sourceID != null && $scope.destinationID != null){
